...
from go_renderer.models import AutoencoderKLWan, AutoencoderKLWan3_8, WanT5EncoderModel
from go_renderer.models.multiref_transformer3d import CroodRefTransformer3DModel2_2
from go_renderer.pipeline import WanFunCroodRefPipeline2_2
from go_renderer.utils.discrete_sampler import DiscreteSampling
from go_renderer.utils.utils import padding_image, save_videos_grid
...
def parse_args():
    ...
    parser.add_argument(
        "--model_name",
        default="models/Diffusion_Transformer/GO-Renderer-5B",
        help="Complete GO-Renderer 5B model directory containing the Transformer, T5, VAE, and tokenizer",
    )
    parser.add_argument(
        "--train_data_dir",
        type=str,
    ...
    parser.add_argument(
        "--train_batch_size", type=int, default=2, help="Batch size per process."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Fallback negative-time RoPE spacing when the complete model config does not define it.",
    )
    parser.add_argument(
        "--coordmap_aug_prob",
        type=float,
    ...
    if args.video_sample_size <= 0 or args.video_sample_size % 32:
        parser.error("--video_sample_size must be a positive multiple of 32")
    if args.image_sample_size <= 0 or args.image_sample_size % 32:
        parser.error("--image_sample_size must be a positive multiple of 32")
    if args.fix_sample_size is not None and any(
        value <= 0 or value % 32 for value in args.fix_sample_size
    ):
        parser.error("--fix_sample_size height and width must be positive multiples of 32")
    if args.validation_size is not None:
        try:
            validate_generation_shape(
...
def main():
    ...
    transformer_kwargs["rope_gap"] = args.rope_gap
    ...
    for transformer_config_path in config_candidates:
        if transformer_config_path.is_file():
            ...
            transformer_kwargs["rope_gap"] = int(
                saved_transformer_config.get("rope_gap", args.rope_gap)
            )
            break
        ...
        if fsdp_stage != 0:
            ...
            def load_model_hook(models, input_dir):
                if args.use_ema:
                    ...
                    _, ema_kwargs = CroodRefTransformer3DModel2_2.load_config(ema_path, return_unused_kwargs=True)
                    load_model = CroodRefTransformer3DModel2_2.from_pretrained(
                        input_dir, subfolder="transformer_ema",
                        transformer_additional_kwargs=transformer_kwargs,
                    )
                    load_model = EMAModel(
                        load_model.parameters(),
                        model_cls=CroodRefTransformer3DModel2_2,
