#!/usr/bin/env python
"""Batch inference for the GO-Renderer Wan2.2 5B model."""
...
from go_renderer.models import AutoTokenizer, AutoencoderKLWan, AutoencoderKLWan3_8, WanT5EncoderModel
...
from go_renderer.utils.utils import filter_kwargs, save_videos_grid
...
def parse_args():
    ...
    parser.add_argument("--config_path", default="config/wan2.2/wan_civitai_5b.yaml")
    parser.add_argument(
        "--model_name",
    ...
    parser.add_argument("--output_dir", default="outputs/go_renderer_5b")
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional override; by default use the complete model's Transformer config, then fall back to 3",
    )
...
def _build_pipeline(args, config, device, weight_dtype):
    ...
    if rope_gap is None:
        rope_gap = _model_rope_gap(args.model_name, transformer_subpath, fallback=3)
...
def main():
    args = parse_args()
    validate_generation_shape(args.height, args.width, args.num_frames, spatial_multiple=32)
    ...
    for index, sample in enumerate(tqdm(manifest, desc="GO-Renderer 5B inference")):
        ref_path = sample["ref"]
